# Attribution

This project, The Book of Robocoding, builds on two decades of work by many people across Robocode and Robocode Tank Royale.

## Primary Authors

- Flemming Nørnberg Larsen — Author of The Book of Robocoding; Robocode maintainer (2005–present); creator of Robocode Tank Royale
- Mathew A. Nelson — Original creator of Robocode (2001–2005)

## Notable Contributors

- Pavel Savara — Major contributions to Robocode (modularization, replay, refactoring, build, and more)
- RoboWiki contributors — Foundational research, analysis, and documentation
- All bot authors, tournament organizers, and knowledge sharers in the Robocode community

## RoboRumble and LiteRumble

- Albert Perez — Creator of RoboRumble
  - Reference: http://robowiki.net/robowiki/RoboRumble
- Julian Kent (Skilgannon) — Creator/maintainer of LiteRumble; host of RoboWiki.net
  - Reference: http://robowiki.net/robowiki/LiteRumble

For more detailed contributor histories and discussions, please consult the respective RoboWiki pages above.

## Source Acknowledgments

Some explanatory text, formulas, and strategy descriptions in this repository are inspired by or derived from articles on RoboWiki.net, which is licensed under CC BY-SA 3.0. When content is adapted, we provide attribution in-page where practical and include page references in NOTICE.md.

- RoboWiki: http://robowiki.net/ — License: CC BY-SA 3.0 (https://creativecommons.org/licenses/by-sa/3.0/)

## Licenses in This Repository

- Documentation (this book): CC BY-SA 4.0 — https://creativecommons.org/licenses/by-sa/4.0/
- Code examples and configuration: MIT — https://opensource.org/licenses/MIT

If you notice missing attributions or have corrections, please open an issue or PR with details and sources.
