...
**Description:** Create a side-by-side comparison:
- **Left (Random Targeting):** Show a bot at center-left. Draw the enemy as a red dot to the upper-right. Draw 5–7 bullet trajectories from the bot, all originating from roughly the same direction (enemy bearing) but scattered at random angles (±20° spread). Label: "Reference bearing" (arrow to enemy), "Random offsets" (arc showing spread).
- **Right (Area Targeting):** Show the same bot and enemy. Draw the enemy's last known position as a red dot. Draw a dashed circle or square around it (the "zone of possible positions"). Draw 5–7 bullet trajectories aimed at random points within that zone. Label: "Last known position", "Zone of uncertainty", "Spray coverage".
**Colors:** Bot = blue (#3B82F6), Enemy = red (#EF4444), Bullets = orange (#F59E0B), Zone = light gray (#D1D5DB) with dashed border (#6B7280)
**Size:** 700×350 px recommended (wide to accommodate side-by-side layout)
-->
