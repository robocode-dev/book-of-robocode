...
**All pages must be saved in UTF-8 encoding, with no escape characters. Emoji characters are allowed and preserved.**
...
- **Planned title** → maps to frontmatter `title`.
    ...
    - Only if **Planned title** is blank or missing, derive a concise, descriptive title from the goal and required
      topics.
